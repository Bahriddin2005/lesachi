...
function BottomNav({ screen, onChange }) {
  return <View style={s.bottomNav}>
    <NavButton active={screen === 'home'} icon="home-outline" label="Asosiy" onPress={() => onChange('home')} />
    <NavButton active={screen === 'customers'} icon="account-group-outline" label="Mijozlar" onPress={() => onChange('customers')} />
    <NavButton active={screen === 'inventory'} icon="warehouse" label="Ombor" onPress={() => onChange('inventory')} />
    <NavButton active={screen === 'history'} icon="history" label="Tarix" onPress={() => onChange('history')} />
    <NavButton active={screen === 'settings'} icon="cog-outline" label="Sozlama" onPress={() => onChange('settings')} />
  </View>;
}

function NavButton({ active, icon, label, onPress }) {
  return <Pressable style={s.navButton} onPress={onPress}><MaterialCommunityIcons name={icon} size={24} color={active ? C.green : C.muted} /><Text style={[s.navLabel, active && s.navActive]}>{label}</Text></Pressable>;
}
...
const s = StyleSheet.create({
  ...
  bottomNav: { position: 'absolute', left: 12, right: 12, bottom: Platform.OS === 'ios' ? 10 : 12, height: 92, borderRadius: 12, backgroundColor: C.white, borderWidth: 1, borderColor: C.line, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-around', paddingHorizontal: 2 }, navButton: { flex: 1, alignItems: 'center', justifyContent: 'center', gap: 6, paddingVertical: 6 }, navIcon: { color: C.muted, fontSize: 31.1 }, navLabel: { color: C.muted, fontSize: 20, fontWeight: '400' }, navActive: { color: C.green, fontWeight: '500' },
